...
def test_check_urls_client_error(mocker: MockerFixture):
   ...
   mock_requests_get.assert_called_once_with(urls[0], timeout=5)
   assert result[urls[0]] == "404 Not Found"
   
@pytest.mark.parametrize("error_exception, expected_status", 
                         [
...
def test_check_urls_request_exception(
   mocker: MockerFixture, error_exception: type[requests.exceptions.RequestException], expected_status:str
):
   mock_requests_get = mocker.patch("simple_http_checker.checker.requests.get")
...
def test_check_urls_with_multiple_urls(mocker: MockerFixture):
   ...
   mock_requests_get.side_effect = [None, timeout_exception]
   ...
   mock_requests_get.side_effect = [mock_response_ok, timeout_exception, mock_response_fail]
...
def test_check_urls_custom_timeout(mocker: MockerFixture):
   ...
   urls = ["http://www.example.com"]
   custom_timeout = 10
   result = check_urls(urls, timeout=custom_timeout)
